    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-16">
        <div class="flex items-center">
          ...
          <span class="ml-3 text-xl font-bold text-blue-400">Playverse</span>
        </div>
        ...
        <div class="flex items-center space-x-2">
          <a href="{{ url('/login') }}" class="text-white border-b-2 border-blue-500 px-3 py-2">Login</a>
          <a href="{{ url('/register') }}" class="btn-neon px-4 py-2 rounded-lg">Sign Up</a>
        </div>
      </div>
    </div>
  </nav>
...
<form method="POST" action="{{ route('login.proses') }}">
    ...
    <div class="space-y-4">
        ...
        <div class="flex justify-between text-sm">
            ...
            <a href="#" class="text-blue-400 hover:text-blue-300">Forgot?</a>
        </div>
        <button type="submit" class="btn-neon w-full py-3 rounded-lg">Sign In</button>
    </div>
</form>
<p class="text-center text-gray-400 mt-6">
    Don’t have an account? <a href="{{ route('register') }}" class="text-blue-400">Sign Up</a>
</p>
    </div>
  </div>
</body>
